...
export class ExpectedSarsaAgent implements RLAgent {
  algorithm: "Expected SARSA" = "Expected SARSA";
  ...
  update(transition: Transition): void {
    ...
    const greedy = argmax(qNext);
    const uniformProb = this.eps / ALL_ACTIONS.length;
    const expectedNext =
      (1 - this.eps) * qNext[greedy] +
      uniformProb * qNext.reduce((sum, value) => sum + value, 0);
    const target = transition.reward + (transition.done ? 0 : this.params.gamma * expectedNext);
